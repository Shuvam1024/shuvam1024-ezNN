...
static void forward_pass(simpleNNType *nn, float *x, float *y) {
...
static void calc_error(simpleNNType *nn, int layer_number, float *expected_out) {
...
static void calc_grad(simpleNNType *nn, int i) {
...
static void calc_param_grads(simpleNNType *nn, int i) {
    for (int j = 0; j < nn->layer_sizes[i]; ++j) {
...
static void update_params(simpleNNType *nn, float learning_rate) {
...
static void back_propogation(simpleNNType *nn, float * data_train, float *expected_out, float learning_rate) {
...
static void initialize_random_params(simpleNNType *nn) {
    srand(time(NULL));
    for (int i = 0; i < nn->nlayers - 1; i++) {
